...
class _HomePageState extends State<HomePage> with RouteAware {
  ...
  Future<void> _loadCalorieGoal() async {
    // First set local value immediately for snappy UI
    final localGoal = await UserDatabase().getDailyCalorieGoalLocal(
    ...
    // Then try to refresh from backend without blocking UI
  ...
  Widget _buildBottomNavigationBar() {
    return BottomNavigationBar(
      ...
      onTap: (index) async {
        if (index == 2) {
          // + button - show food logging screen
