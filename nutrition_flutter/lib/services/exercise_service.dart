...
class ExerciseService {
  ...
  static Future<bool> logExerciseSession({
    ...
    try {
      final uri = Uri.parse('$apiBase/exercises/session');
      ...
      if (caloriesBurned != null && caloriesBurned > 0) {
        body['calories_burned'] = caloriesBurned;
      }
      ...
      debugPrint('🔄 Logging exercise session:');
      debugPrint('   User: $user');
      debugPrint('   Exercise ID: $exerciseId');
      debugPrint('   Exercise Name: $exerciseName');
      debugPrint('   Duration: $durationSeconds seconds');
      debugPrint('   Calories: $caloriesBurned');
      ...
      if (resp.statusCode == 200) {
        try {
          ...
          if (success) {
            ...
            debugPrint('❌ Backend returned success=false: ${data['error'] ?? data['message'] ?? 'Unknown error'}');
          }
          return success;
        } catch (e) {
      ...
      debugPrint('❌ EXERCISE DEBUG: log exercise session failed: $e');
  ...
  static Future<Map<String, dynamic>> getExerciseSessions({
    ...
    try {
      var uri = Uri.parse('$apiBase/exercises/sessions?user=$user');
      if (date != null) {
        uri = Uri.parse('$apiBase/exercises/sessions?user=$user&date=$date');
      }
      ...
      debugPrint('EXERCISE DEBUG: get exercise sessions failed: $e');
      return {
        'success': false,
        'sessions': [],
        'summary': {
  ...
  static Future<bool> deleteExerciseSession(int sessionId) async {
    try {
      final uri = Uri.parse('$apiBase/exercises/session/$sessionId');
      
      debugPrint('🗑️ Deleting exercise session: $sessionId');
      debugPrint('   URL: $uri');
      
      final resp = await http.delete(uri).timeout(const Duration(seconds: 10));
      
      debugPrint('📡 Delete response status: ${resp.statusCode}');
      debugPrint('📡 Delete response body: ${resp.body}');
      ...
      if (resp.statusCode == 200) {
        try {
          ...
          debugPrint('❌ Failed to parse delete response: $e');
          return false;
        }
      } else {
        debugPrint('❌ HTTP error ${resp.statusCode}: ${resp.body}');
      ...
      debugPrint('❌ EXERCISE DEBUG: delete exercise session failed: $e');
      debugPrint('   Stack trace: $stackTrace');
      return false;
    }
  }
